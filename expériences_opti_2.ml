...
let tableau_opti_2 l=
  ...
  let rec remplissage q=match q with
    ...
    |(a,b,c,d)::t when a<b-> croiss.(a)<-(b,d)::croiss.(a); decroiss.(b)<-(a,oppose c)::decroiss.(b) ;remplissage t
 	 |(a,b,c,d)::t when a>b-> decroiss.(a)<-(b,d)::decroiss.(a); croiss.(b)<-(a,oppose c)::croiss.(b) ;remplissage t
  in 
  remplissage q;;
tableau_opti_2 l;;
...
let bellman_ford_opti_2 (n,tab_croiss,tab_decroiss) s=
  let dist=Array.make n Inf in
  dist.(s)<-Entier(0);
  let modif_0=Array.make n false in
  modif_0.(0)<-true;
  let modif_1=Array.make n false in
  modif_1.(0)<-true;
  let rec maj liste i= match liste with
    |[]->()
    |(a,b)::q when minore (add dist.(i) b) dist.(a) ->
        (dist.(a)<-add dist.(i) b);modif_0.(a)<-true;modif_1.(a)<-true;
        maj q i 
    |(a,b)::q->maj q i 
  in
  ...
  in 
  let k= ref 0 in 
  let disjoncteur= ref true in
  while !k <n && !disjoncteur do 
  	 disjoncteur:=false;
    for i=0 to (n-1) do 
    	if modif_0.(i) then (maj tab_croiss.(i) i;modif_0.(i)<-false)
	 done;
	 for i=0 to (n-1) do 
    	if modif_1.(n-1-i) then (maj tab_decroiss.(n-1-i) (n-1-i);
    	modif_1.(n-1-i)<-false;)
	 done;
	 let i=ref 0 in
	 while !i<n && not !disjoncteur do
	 	disjoncteur:=modif_0.(!i)|| modif_1.(!i);
	 	i:= !i+1;
	 done;
    k:= !k+1;
  done;
...
let tab nb sommets dist=
	...
	for i=0 to nb-1 do 
		t.(i)<-tableau_opti_2(gene_aleat sommets dist)
	done;
	t;;
...
let mesure t=
	...
	for i=0 to (Array.length t)-1 do
		ignore (bellman_ford_opti_2 t.(i) 0)
	done;
	Sys.time()-.t1;;

let duree_totale nb_test nb_sommets distance_carac=
	let t=tab nb_test nb_sommets distance_carac in
	mesure (t);;
...
bellman_ford_opti_2 (tableau_opti_2 d) 0;;
gene_aleat 5 20;;
tab 1 90 100;;
tableau_opti_2(d);;
